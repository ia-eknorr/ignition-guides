...
Pods come and go. Kubernetes can move them between [**nodes**](https://kubernetes.io/docs/concepts/architecture/nodes/) (the machines in the cluster, physical or virtual, similar to a server or VM in a traditional Ignition install), restart them if they crash, or replace them when configuration changes. Anything that needs to live longer than a single pod has to live somewhere else (persistent volumes for data, Services for networking, and so on). The rest of this guide is about those "somewhere else" pieces.
